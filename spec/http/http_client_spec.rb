...
describe Reach::HTTP::Client do
  before do
    @client = Reach::HTTP::Client.new
  end
  ...
  it 'should allow setting a global timeout' do
    @client = Reach::HTTP::Client.new(timeout: 10)
    @connection = Faraday::Connection.new
  ...
  it 'should contain a last response' do
    ...
    expect(@client.last_response.is_a?(Reach::Response)).to be(true)
  ...
  it 'should contain a last request' do
    ...
    expect(@client.last_request.is_a?(Reach::Request)).to be(true)
  ...
  it 'should contain a last_response but no response on a connection error' do
    ...
    expect { @client.request('host', 'port', 'GET', 'url', nil, nil, {}, ['a', 'b']) }.to raise_exception(Reach::REST::ReachError)
  ...
  describe 'last_response' do
    let(:last_response) { Reach::Response.new(200, 'body') }
  end
  ...
  context 'when there is a server error' do
    ...
    let(:reach_response) { @client.request('host', 'port', 'GET', 'url') }
    ...
    (500..599).each do |status_code|
      it 'generates a JSON-parseable body' do
        ...
        expect(reach_response.body).to eq({ 'message' => "Server error (#{status_code})", 'code' => status_code })
        expect(reach_response.status_code).to eq status_code
      end
    end
  end
end
