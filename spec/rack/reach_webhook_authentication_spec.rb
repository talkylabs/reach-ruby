...
describe Rack::ReachWebhookAuthentication do
  before do
  ...
  describe 'new' do
    it 'should initialize with an app, auth token and a path' do
      expect do
        Rack::ReachWebhookAuthentication.new(@app, 'ABC', /\/voice/)
      end.not_to raise_error
    end

    it 'should initialize with an app, auth token and paths' do
      expect do
        Rack::ReachWebhookAuthentication.new(@app, 'ABC', /\/voice/, /\/sms/)
      end.not_to raise_error
    end

    it 'should initialize with an app, dynamic token and paths' do
      expect do
        Rack::ReachWebhookAuthentication.new(@app, nil, /\/voice/, /\/sms/)
      end.not_to raise_error
    end
  end
  ...
  describe 'calling against one path with dynamic auth token' do
    it 'should allow a request through if it validates' do
      ...
      @middleware = Rack::ReachWebhookAuthentication.new(@app, nil, /\/voice/) { |asid| auth_token }
      request_validator = double('RequestValidator')
      expect(Reach::Security::RequestValidator).to receive(:new).with(auth_token).and_return(request_validator)
  ...
  describe 'calling against one path' do
    before do
      @middleware = Rack::ReachWebhookAuthentication.new(@app, 'ABC', /\/voice/)
    end

    it 'should not intercept when the path doesn\'t match' do
      expect(Reach::Security::RequestValidator).to_not receive(:validate)
    ...
    it 'should allow a request through if it validates' do
      expect_any_instance_of(Reach::Security::RequestValidator).to(
  ...
  describe 'calling against many paths' do
    before do
      @middleware = Rack::ReachWebhookAuthentication.new(@app, 'ABC', /\/voice/, /\/sms/)
    end
  ...
  describe 'validating non-form-data POST payloads' do
    it 'should fail if the body does not validate' do
      middleware = Rack::ReachWebhookAuthentication.new(@app, 'qwerty', /\/test/)
      ...
      request['HTTP_X_REACH_SIGNATURE'] = '+LYlbGr/VmN84YPJQCuWs+9UA7E='
    ...
    it 'should validate if the body signature is correct' do
      ...
      request['HTTP_X_REACH_SIGNATURE'] = 'zR5Oq4f6cijN5oz5bisiVuxYnTU='
      request['CONTENT_TYPE'] = 'application/json'
  ...
  describe 'validating application/x-www-form-urlencoded POST payloads' do
    it 'should fail if the body does not validate' do
      ...
      request['HTTP_X_REACH_SIGNATURE'] = 'foobarbaz'
    ...
    it 'should validate if the body signature is correct' do
      ...
      request['HTTP_X_REACH_SIGNATURE'] = 'TR9Skm9jiF4WVRJznU5glK5I83k='
      expect(request['CONTENT_TYPE']).to eq('application/x-www-form-urlencoded')
